...
abstract class Settings
{
    ...
    public function output_tabs($tabs)
    {
        foreach ($tabs as $tab_id => $tab_name) {
            ?>
			<a href="<?php 
            echo esc_url(admin_url('admin.php?page=' . $this->get_current_page() . '&tab=' . $tab_id));
            ?>" class="nav-tab <?php 
            echo esc_attr($this->get_current_tab() === $tab_id ? 'nav-tab-active' : '');
            ?>">
				<?php 
    ...
    protected function output_form($settings)
    {
        if (!empty($settings)) {
            ...
            ?>
				<?php 
            wp_nonce_field($class_name);
            ?>
				<?php 
            submit_button(null, 'primary', $class_name);
            ?>
			</form>
			<?php 
        }
    ...
    protected function output_plugins_widget()
    {
        ...
        if (!empty($promo_plugins)) {
            ...
            foreach ($promo_plugins as $promo_plugin) {
                $promo_plugin = wp_parse_args($promo_plugin, array('name' => '', 'description' => '', 'basename' => '', 'slug' => '', 'badge' => esc_html__('Recommended', 'wc-variation-images'), 'button' => esc_html__('Install Now', 'wc-variation-images'), 'installed' => false));
                ...
                $install_url = add_query_arg(array('action' => 'install-plugin', 'plugin' => $slug), network_admin_url('update.php'));
                $install_url = wp_nonce_url($install_url, 'install-plugin_' . $slug);
                ?>
				<div class="bk-card">
					<div class="bk-card__header">
						<?php 
                echo esc_html($promo_plugin['name']);
                ?>
					</div>
					<div class="bk-card__body">
						<?php 
                echo wp_kses_post(wpautop($promo_plugin['description']));
                ?>
					</div>
					<div class="bk-card__footer">
						<?php 
                if (!empty($promo_plugin['badge'])) {
                    ?>
							<span class="bk-badge bk-badge--primary">
								<?php 
                    echo esc_html($promo_plugin['badge']);
                    ?>
							</span>
						<?php 
                }
                ...
                ?>" target="_blank" class="bk-button bk-button--secondary">
							<?php 
                echo esc_html($promo_plugin['button']);
                ?>
						</a>
					</div>
				</div>
    ...
    protected function output_support_widget()
    {
        ...
        if (!empty($support_links)) {
			...
			<div class="bk-card">
				<div class="bk-card__header">
					<?php 
            esc_html_e('Need Help?', 'wc-variation-images');
            ?>
				</div>
				<div class="bk-card__body">
					<ul>
						<?php 
            foreach ($support_links as $key => $link) {
                ?>
							<li>
								<a href="<?php 
    ...
    {
        return array(array('name' => 'WC Min Max Quantities', 'slug' => 'wc-min-max-quantities', 'description' => 'Set minimum and maximum price or quantity for WooCommerce products.', 'link' => 'https://wordpress.org/plugins/wc-min-max-quantities/', 'badge' => esc_html__('Recommended', 'wc-variation-images'), 'button' => esc_html__('Install Now', 'wc-variation-images')));
    }
    /**
     * Get support links.
     *
    ...
    {
        return array('facebook' => array('label' => __('Join our Community', 'wc-variation-images'), 'url' => 'https://www.facebook.com/groups/pluginever'), 'feature-request' => array('label' => __('Request a Feature', 'wc-variation-images'), 'url' => 'https://www.pluginever.com/contact/'), 'bug-report' => array('label' => __('Report a Bug', 'wc-variation-images'), 'url' => 'https://www.pluginever.com/contact/'));
    }
    /**
     * Get current page.
     *
